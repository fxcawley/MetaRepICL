...
def route_a_end2end(seed: int = 123, k: int = 32, p: int = 16, d_proj: int = 12, tau: float = 0.5, lam: float = 1e-2, noise: float = 0.1):
	...
	w_true = torch.randn(p, dtype=torch.float64) / math.sqrt(p)
	y_s = (phi_s @ w_true) + noise * torch.randn(k, dtype=torch.float64)
	y_true = (phi_q @ w_true).squeeze(0)
	# Project features for exponential kernel
